var IndexCardDrawingUtils = {};


IndexCardDrawingUtils.complexBaseSize = 15;
IndexCardDrawingUtils.proteinRadius = function(p){return 7};
IndexCardDrawingUtils.complexSize = function(c){
    if(c._expanded){
        return c._expandedSize;
    }else return IndexCardDrawingUtils.complexBaseSize};

IndexCardDrawingUtils.link = function (link, pathway) {
    var thick = 2,
    ...
    if(link.source.type == "complex"){
        start = start.addV(direction.mulS(IndexCardDrawingUtils.complexSize(link.source)*0.5));
    } else if (link.source.type == "protein"){
        start = start.addV(direction.mulS(IndexCardDrawingUtils.proteinRadius(link.source)*0.5));
    }

    //end shift

    if(link.source.type == "complex"){
        end = end.subV(direction.mulS(IndexCardDrawingUtils.complexSize(link.target)*0.5));
    } else if (link.source.type == "protein"){
        end = end.subV(direction.mulS(IndexCardDrawingUtils.proteinRadius(link.target)*0.5));
    }
...
IndexCardDrawingUtils.linkArch = function (link, pathway) {
...
IndexCardDrawingUtils.pathFromLabelToNode = function(d){
